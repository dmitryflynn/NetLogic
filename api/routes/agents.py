...
async def agent_heartbeat(
    ...
    return {
        ...
        "cancel_job_ids": job_manager.cancel_ids_for_agent(agent_id),
    }
...
async def submit_events(
    ...
    if job.status in ("cancelled", "completed", "failed"):
        raise HTTPException(
            status_code=409,
            detail=f"Job is {job.status}; events are no longer accepted.",
        )
...
async def complete_task(
    ...
    * If `error` is set → job is marked `failed` with the error message.
    ...
    terminal = "failed" if payload.error else "completed"
    error = payload.error or ""
    job, changed = job_manager.try_set_terminal(
        job_id, terminal, org_id=agent.org_id if agent.org_id else "", error=error,
    )
    if job is None:
        raise HTTPException(status_code=404, detail=f"Job '{job_id}' not found.")
