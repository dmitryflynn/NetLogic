...
- Prefer proof tools: http_request, dir_enum, browser_get (JS challenges), ssdp_discover/udp_probe,
  timing_probe, confirm_tech.
...
3. Prefer discriminating checks: custom headers/body, non-root paths, timing_probe, raw_tcp on open ports,
   crash_probe ONLY if listed and needed for http.sys-class CVEs.
...
7. READ-ONLY: never mutate target data/content. http methods are GET/HEAD/OPTIONS only.
