...
# ── AIProposer (optional augmentation; fake completer, offline) ─────────────────────
def test_ai_proposer_augments_hypotheses():
    import json
    from src.reasoning.ai_proposer import AIProposer
    s = _state_with_service()
    generators.populate_objectives(s)            # creates identify_framework:ex.com:80
    fake = lambda sysp, usr: json.dumps(
        [{"objective": "identify_framework:ex.com:80", "candidates": {"spring_boot": 0.7, "django": 0.3}}])
    added = AIProposer(fake).augment_hypotheses(s)
    assert added == 1
    ai_hyps = [h for h in s.investigation.hypotheses.all() if h.created_by == "ai"]
    assert ai_hyps and "spring_boot" in ai_hyps[0].likelihoods


def test_ai_proposer_intents_are_scoped_to_target():
    import json
    from src.reasoning.ai_proposer import AIProposer
    s = _state_with_service()
    fake = lambda sysp, usr: json.dumps(["http_headers", "server_header"])
    intents = AIProposer(fake).propose_intents(s)
    assert intents
    assert all(i.target_ref == "ex.com" for i in intents)          # never an AI-named host
    ev = {e.value for i in intents for e in i.desired_evidence}
    assert "http_headers" in ev


def test_ai_proposer_is_failsoft_on_bad_output():
    from src.reasoning.ai_proposer import AIProposer
    s = _state_with_service()
    generators.populate_objectives(s)
    bad = lambda sysp, usr: "this is not json"
    assert AIProposer(bad).augment_hypotheses(s) == 0
    assert AIProposer(bad).propose_intents(s) == []


def test_ai_proposer_ignores_unknown_evidence_types():
    import json
    from src.reasoning.ai_proposer import AIProposer
    s = _state_with_service()
    fake = lambda sysp, usr: json.dumps(["http_headers", "exfiltrate_secrets", "rm -rf"])
    intents = AIProposer(fake).propose_intents(s)
    ev = {e.value for i in intents for e in i.desired_evidence}
    assert ev == {"http_headers"}                                  # bogus types dropped
