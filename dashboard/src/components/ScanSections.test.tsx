...
describe('ScanSections — tab views', () => {
  ...
  it('executive view: full AI report + all analysis panels, no raw evidence', () => {
    render(<ScanSections s={s} view="executive" />)
    expect(screen.getByText('Vulnerabilities')).toBeInTheDocument()
    expect(screen.getByText(/Overall this host is risky/)).toBeInTheDocument()   // exec summary
    expect(screen.getByText(/detail here/)).toBeInTheDocument()                  // full report incl. findings
    expect(screen.getByText('Vulnerability detail')).toBeInTheDocument()
    expect(screen.queryByText('TLS / Certificate')).not.toBeInTheDocument()
  })

  it('technical view: renders no summary panels (the deep-dive is generated by the page)', () => {
